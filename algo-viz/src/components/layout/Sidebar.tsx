...
import type { Algorithm, AlgorithmGroup } from '../../types';
...
export default function Sidebar() {
  ...
  return (
    <aside className="w-64 flex-shrink-0 bg-slate-900/80 border-r border-slate-800 flex flex-col h-full overflow-hidden">
      {/* Header */}
      ...
      <nav className="flex-1 overflow-y-auto py-2">
        {algorithmGroups.map((group: AlgorithmGroup) => (
          <div key={group.id}>
            <button
              onClick={() => toggleGroup(group.id)}
              className="w-full flex items-center gap-2 px-4 py-2 text-left hover:bg-slate-800/60 transition-colors"
            >
              <span className="text-base w-5 text-center text-slate-400" style={{ fontFamily: 'monospace' }}>
                {group.icon}
              </span>
            ...
            <AnimatePresence>
              {expanded.has(group.id) && (
                ...
                >
                  {group.algorithms.map((algo: Algorithm) => {
      ...
      <div className="px-4 py-3 border-t border-slate-800">
        <div className="text-[10px] text-slate-600 text-center">
          {algorithmGroups.reduce((acc, g) => acc + g.algorithms.length, 0)} algorithms visualized
        </div>
      </div>
    </aside>
  );
